...
export function isValidDate(dateString,strict=false) {
  return (!strict && moment(dateString).isValid()) || moment(dateString,['YYYY-MM-DD','DDMMYYYY','DD/MM/YYYY'],true).isValid()
}

export function isValidDateTime(dateTimeString,strict=false) {
  return (!strict && moment(dateTimeString).isValid()) || moment(dateTimeString,['YYYY-MM-DD HH:mm','DDMMYYYYHHmm','DD/MM/YYYY HHmm'],true).isValid()
}
...
export function parseDateTime(dateTimeString) {
  if (helper.isValidDateTime(dateTimeString)) {
    let tmpDate = (moment(dateTimeString,['YYYY-MM-DD HH:mm','DDMMYYYYHHmm','DD/MM/YYYY HHmm'],true).isValid()) ? moment(dateTimeString,['YYYY-MM-DD HH:mm','DDMMYYYYHHmm','DD/MM/YYYY HHmm'],true) :  moment(dateTimeString)
    if (moment.duration(tmpDate.diff(moment())).years() > 300) {
      tmpDate = tmpDate.toObject()
      tmpDate.years = tmpDate.years - 543
    }
